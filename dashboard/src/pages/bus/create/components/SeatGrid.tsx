...
import { LuArmchair, LuTrash, LuTrash2 } from "react-icons/lu";
import { MdOutlineCleaningServices, MdOutlineStairs } from "react-icons/md";
import { cn } from "@/utils/cn";

interface SeatGridProps {
  floorIndex: number;
  control: Control<CreateBusSchemaType>;
}

const SEAT_TYPES = [
  { value: "asiento", label: "Asiento", icon: <LuArmchair size={16} />, color: "success" },
  { value: "limpieza", label: "Limpieza", icon: <MdOutlineCleaningServices size={16} />, color: "warning" },
  { value: "escalera", label: "Escalera", icon: <MdOutlineStairs size={16} />, color: "danger" },
] as const;

export default function SeatGrid({ floorIndex, control }: SeatGridProps) {
  ...
  const columns = useWatch({ control, name: `floors.${floorIndex}.columns` }) || 0;

  const getSeatAt = (row: number, col: number) => {
    return fields.find((s) => s.row === row && s.column === col);
  };

  const getSeatIndexAt = (row: number, col: number) => {
    return fields.findIndex((s) => s.row === row && s.column === col);
  };
  ...
  const renderCell = (row: number, col: number) => {
    ...
    if (!seat) {
      return (
        ...
        >
          <span className="text-[10px] text-default-400 font-mono">{row},{col}</span>
        </div>
      );
    }
    ...
    return (
      <Popover key={`${row}-${col}`} placement="bottom" showArrow={true}>
        <PopoverTrigger>
          <div
            className={cn(
              "w-12 h-12 rounded-lg flex flex-col items-center justify-center cursor-pointer transition-all shadow-sm border-2",
              seat.typeSeat === "asiento" && "bg-success-50 border-success-200 text-success-600 hover:bg-success-100",
              seat.typeSeat === "limpieza" && "bg-warning-50 border-warning-200 text-warning-600 hover:bg-warning-100",
              seat.typeSeat === "escalera" && "bg-danger-50 border-danger-200 text-danger-600 hover:bg-danger-100"
            )}
          >
            {typeConfig?.icon}
        ...
        <PopoverContent className="p-4 w-64 gap-3 bg-content1 border-default-200">
          <div className="w-full flex flex-col gap-3">
            <p className="text-sm font-semibold border-b pb-2">Configurar Celda ({row}, {col})</p>
            
            <Select
              label="Tipo"
              size="sm"
              selectedKeys={[seat.typeSeat]}
              onChange={(e) => update(seatIndex, { ...seat, typeSeat: e.target.value as any })}
            >
              {SEAT_TYPES.map((type) => (
            ...
            {seat.typeSeat === "asiento" && (
              <Input
                ...
                placeholder="Ej: 01"
                value={seat.name}
                onChange={(e) => update(seatIndex, { ...seat, name: e.target.value })}
              />
            )}
